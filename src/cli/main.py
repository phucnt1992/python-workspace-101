...
def list_todos() -> None:
    async def _list() -> list[Todo]:
        ...
        async with get_session_context() as session:
            return list(await get_todo_list(session))
...
def complete(todo_id: int = typer.Argument(..., help="ID of the todo item to mark as completed")) -> None:
    async def _complete() -> Todo | None:
        await init_db()
        async with get_session_context() as session:
            return await set_todo_completed(session, todo_id, completed=True)
...
def reopen(todo_id: int = typer.Argument(..., help="ID of the todo item to reopen")) -> None:
    async def _reopen() -> Todo | None:
        await init_db()
        async with get_session_context() as session:
            return await set_todo_completed(session, todo_id, completed=False)
...
def get_todo(todo_id: int = typer.Argument(..., help="ID of the todo item to retrieve")) -> None:
    async def _get() -> Todo | None:
        await init_db()
        async with get_session_context() as session:
            return await get_todo_by_id(session, todo_id)

    todo = asyncio.run(_get())
    if todo is None:
        typer.echo(f"Todo with id={todo_id} was not found.")
        raise typer.Exit(code=1)
    typer.echo(_format_todo(todo))
