...
def create(
    ...
    async def _create() -> Todo:
        await init_db()
        async with get_session_context() as session:
            return await create_todo(session, title=title, description=description)

    todo = asyncio.run(_create())
    typer.echo("Created todo:")
    typer.echo(_format_todo(todo))
...
def update(
    ...
    async def _update() -> Todo | None:
        await init_db()
        async with get_session_context() as session:
            return await update_todo(session, todo_id=todo_id, title=title, description=description)
...
def delete(todo_id: int = typer.Argument(..., help="ID of the todo item to delete")) -> None:
    async def _delete() -> bool:
        await init_db()
        async with get_session_context() as session:
            return await delete_todo(session, todo_id=todo_id)
...
def complete(todo_id: int = typer.Argument(..., help="ID of the todo item to mark as completed")) -> None:
    async def _complete() -> Todo | None:
        await init_db()
        async with get_session_context() as session:
            return await set_todo_completed(session, todo_id=todo_id, completed=True)
...
def reopen(todo_id: int = typer.Argument(..., help="ID of the todo item to reopen")) -> None:
    async def _reopen() -> Todo | None:
        await init_db()
        async with get_session_context() as session:
            return await set_todo_completed(session, todo_id=todo_id, completed=False)
