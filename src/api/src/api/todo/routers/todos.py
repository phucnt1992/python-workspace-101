...
from api.todo.schemas.todo import (
    TodoCreateRequest,
    TodoListResponse,
    TodoResponse,
    TodoUpdateRequest,
...
def _todo_response(todo: object) -> TodoResponse:
    return TodoResponse.model_validate(todo, from_attributes=True)


async def _get_todo_or_404(todo_id: int, db: DbSessionDep) -> TodoResponse:
...
async def _apply_completion(
    ...
    todo = await set_todo_completed(db, todo_id, completed)
    if todo is None:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail="Todo not found",
...
async def list_todos(
    ...
    return TodoListResponse(
        items=[_todo_response(todo) for todo in paged_todos],
        total=total,
        page=page,
        page_size=page_size,
    )
...
async def get_todo(todo_id: int, db: DbSessionDep) -> TodoResponse:
    return await _get_todo_or_404(todo_id, db)
