...
async def get_todo(todo_id: int, db: DbSessionDep) -> TodoResponse:
    ...
    if not todo:
        logger.warning(
            f"Todo with id={todo_id} was not found.", extra={"todo_id": todo_id, "operation": "get", "source": "api"}
        )
...
async def complete_todo(todo_id: int, db: DbSessionDep) -> TodoResponse:
    todo = await set_todo_completed(db, todo_id, completed=True)
    if not todo:
        logger.warning(
            f"Todo with id={todo_id} was not found.",
            extra={"todo_id": todo_id, "operation": "complete", "source": "api"},
        )
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Todo not found")
...
async def reopen_todo(todo_id: int, db: DbSessionDep) -> TodoResponse:
    todo = await set_todo_completed(db, todo_id, completed=False)
    if not todo:
        logger.warning(
            f"Todo with id={todo_id} was not found.", extra={"todo_id": todo_id, "operation": "reopen", "source": "api"}
        )
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Todo not found")
