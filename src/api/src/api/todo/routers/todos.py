...
async def update_todo_endpoint(todo_id: int, payload: TodoUpdateRequest, db: DbSessionDep) -> TodoResponse:
    todo = await update_todo(db, todo_id, title=payload.title, description=payload.description)
    if todo is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Todo not found")
    return TodoResponse.model_validate(todo, from_attributes=True)
...
async def delete_todo_endpoint(todo_id: int, db: DbSessionDep) -> None:
    todo = await delete_todo(db, todo_id)
    if todo is False:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Todo not found")
...
async def complete_todo_endpoint(todo_id: int, db: DbSessionDep) -> TodoResponse:
...
async def reopen_todo_endpoint(todo_id: int, db: DbSessionDep) -> TodoResponse:
