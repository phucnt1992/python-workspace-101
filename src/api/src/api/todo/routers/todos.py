...
async def get_todo(todo_id: int, db: DbSessionDep) -> TodoResponse:
    ...
    if todo is None:
        logger.warning("todo.not_found", extra={"todo_id": todo_id, "operation": "get"})
...
async def create_todo_endpoint(payload: TodoCreateRequest, db: DbSessionDep) -> TodoResponse:
    ...
    logger.info("todo.created", extra={"todo_id": todo.id, "todo_title": todo.title})
    return TodoResponse.model_validate(todo, from_attributes=True)
...
async def update_todo_endpoint(todo_id: int, payload: TodoUpdateRequest, db: DbSessionDep) -> TodoResponse:
    ...
    if todo is None:
        logger.warning("todo.not_found", extra={"todo_id": todo_id, "operation": "update"})
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Todo not found")
    logger.info("todo.updated", extra={"todo_id": todo.id, "todo_title": todo.title})
    return TodoResponse.model_validate(todo, from_attributes=True)
...
async def delete_todo_endpoint(todo_id: int, db: DbSessionDep) -> None:
    ...
    if not deleted:
        logger.warning("todo.not_found", extra={"todo_id": todo_id, "operation": "delete"})
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Todo not found")
    logger.info("todo.deleted", extra={"todo_id": todo_id})
...
async def complete_todo(todo_id: int, db: DbSessionDep) -> TodoResponse:
    ...
    if todo is None:
        logger.warning("todo.not_found", extra={"todo_id": todo_id, "operation": "complete"})
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Todo not found")
    logger.info("todo.completed", extra={"todo_id": todo.id})
    return TodoResponse.model_validate(todo, from_attributes=True)
...
async def reopen_todo(todo_id: int, db: DbSessionDep) -> TodoResponse:
    ...
    if todo is None:
        logger.warning("todo.not_found", extra={"todo_id": todo_id, "operation": "reopen"})
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Todo not found")
    logger.info("todo.reopened", extra={"todo_id": todo.id})
    return TodoResponse.model_validate(todo, from_attributes=True)
